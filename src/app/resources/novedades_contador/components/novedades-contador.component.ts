import { Component, OnInit, ViewChild, inject, ChangeDetectorRef, ViewEncapsulation, EventEmitter, Output } from '@angular/core';
import { CommonModule } from '@angular/common';
import { FormBuilder, FormGroup, ReactiveFormsModule, Validators, FormsModule } from '@angular/forms';
import { MatTableDataSource, MatTableModule } from '@angular/material/table';
import { animate, state, style, transition, trigger } from '@angular/animations';
import { MatPaginator, MatPaginatorModule } from '@angular/material/paginator';
import { MatFormFieldModule } from '@angular/material/form-field';
import { MatSelectModule } from '@angular/material/select';
import { MatInputModule } from '@angular/material/input';
import { MatButtonModule } from '@angular/material/button';
import { MatIconModule } from '@angular/material/icon';
import { MatProgressSpinnerModule } from '@angular/material/progress-spinner';
import { MatProgressBarModule } from '@angular/material/progress-bar';
import { MatTabsModule } from '@angular/material/tabs';
import { MatExpansionModule } from '@angular/material/expansion';
import { MatTooltipModule } from '@angular/material/tooltip';

import { FuseMediaWatcherService } from '@fuse/services/media-watcher';
import { NotificationService } from '@fwk/services/notification/notification.service';
import { TranslatePipe } from '@fwk/pipe/translate.pipe';
import { CustomPageComponent } from '@fwk/model/page-component.interface';
import { ActionDef } from '@fwk/model/component-def/action-def';
import { I18nService } from '@fwk/services/i18n-service/i18n.service';

import { NovedadesContadorService } from '../novedades-contador.service';
import { NovedadesContadorData, Jornal, FueraConvenio, Mensual } from '../novedades-contador.models';

@Component({
    selector: 'app-novedades-contador',
    standalone: true,
    imports: [
        CommonModule,
        ReactiveFormsModule,
        FormsModule,
        MatTableModule,
        MatFormFieldModule,
        MatSelectModule,
        MatInputModule,
        MatButtonModule,
        MatIconModule,
        MatProgressSpinnerModule,
        MatPaginatorModule,
        MatProgressBarModule,
        MatExpansionModule,
        MatTabsModule,
        MatTooltipModule,
        TranslatePipe
    ],
    templateUrl: './novedades-contador.component.html',
    styleUrls: ['./novedades-contador.component.scss'],
    encapsulation: ViewEncapsulation.None,
    animations: [
        trigger('detailExpand', [
            state('collapsed, void', style({ height: '0px', minHeight: '0', display: 'none' })),
            state('expanded', style({ height: '*' })),
            transition('expanded <=> collapsed', animate('225ms cubic-bezier(0.4, 0.0, 0.2, 1)')),
        ]),
    ],
})
export class NovedadesContadorComponent implements OnInit, CustomPageComponent {
    @Output() actionStateChange = new EventEmitter<{ key: string, changes: Partial<ActionDef> }>();

    private _fb = inject(FormBuilder);
    private _notificationService = inject(NotificationService);
    private _novedadesService = inject(NovedadesContadorService);
    private _i18nService = inject(I18nService);
    private _cdr = inject(ChangeDetectorRef);
    private _fuseMediaWatcherService = inject(FuseMediaWatcherService);

    novedadesForm: FormGroup;
    meses: { value: number, viewValue: string }[];
    currentYear = new Date().getFullYear();
    loading = false;
    saving = false;
    panelOpenState = true;
    isMobile = false;

    i18nName = 'NOVEDADES_CONTADOR_I18N_DEF';

    novedadesData: NovedadesContadorData | null = null;

    jornalesDataSource = new MatTableDataSource<Jornal>();
    fueraConvenioDataSource = new MatTableDataSource<FueraConvenio>();
    mensualDataSource = new MatTableDataSource<Mensual>();

    commonDisplayedColumns = ['expand', 'legajo', 'nombreCompleto', 'cuil', 'novedades'];

    expandedJornal: Jornal | null;
    expandedFuera: FueraConvenio | null;
    expandedMensual: Mensual | null;

    @ViewChild('jornalesPaginator') jornalesPaginator: MatPaginator;
    @ViewChild('fueraConvenioPaginator') fueraConvenioPaginator: MatPaginator;
    @ViewChild('mensualPaginator') mensualPaginator: MatPaginator;

    ngOnInit(): void {
        const currentDate = new Date();
        this.novedadesForm = this._fb.group({
            mes: [currentDate.getMonth() + 1, Validators.required],
            anio: [currentDate.getFullYear(), [Validators.required, Validators.min(2001), Validators.max(this.currentYear)]]
        });

        this.meses = [
            { value: 1, viewValue: 'Enero' }, { value: 2, viewValue: 'Febrero' },
            { value: 3, viewValue: 'Marzo' }, { value: 4, viewValue: 'Abril' },
            { value: 5, viewValue: 'Mayo' }, { value: 6, viewValue: 'Junio' },
            { value: 7, viewValue: 'Julio' }, { value: 8, viewValue: 'Agosto' },
            { value: 9, viewValue: 'Septiembre' }, { value: 10, viewValue: 'Octubre' },
            { value: 11, viewValue: 'Noviembre' }, { value: 12, viewValue: 'Diciembre' }
        ];

        this._fuseMediaWatcherService.onMediaChange$
            .subscribe(({ matchingAliases }) => {
                this.isMobile = !matchingAliases.includes('md');
                if (this.isMobile) this.panelOpenState = false;
            });
    }

    onAction(action: ActionDef): void {
        if (action.actionType === 'custom_save_novedades') {
            this.guardarNovedades();
        }
    }

    private translate(key: string): string {
        return this._i18nService.getDictionary(this.i18nName)?.translate?.(key) || key;
    }

    limpiarFiltros(): void {
        const currentDate = new Date();
        this.novedadesForm.patchValue({
            mes: currentDate.getMonth() + 1,
            anio: currentDate.getFullYear()
        });
        this.resetData();
        this.toggleSaveButton(false);
        this.panelOpenState = true;
    }

    private resetData(): void {
        this.novedadesData = null;
        this.jornalesDataSource.data = [];
        this.fueraConvenioDataSource.data = [];
        this.mensualDataSource.data = [];
    }

    vistaPrevia(): void {
        if (this.novedadesForm.invalid) return;

        this.toggleSaveButton(false);
        this.loading = true;
        this.resetData();

        const { anio, mes } = this.novedadesForm.value;

        this._novedadesService.getPreview(anio, mes).subscribe({
            next: (response) => {
                const data = response.data;
                if (!data || (!data.jornales.length && !data.fueraConvenio.length && !data.mensual.length)) {
                    this._notificationService.notify(this.translate('novedades_contador_no_hay_resultados'));
                    this.loading = false;
                    this._cdr.markForCheck();
                    return;
                }

                this.novedadesData = data;

                const initNovedades = (item: any) => ({ ...item, novedades: item.novedades || '' });

                this.jornalesDataSource.data = data.jornales.map(initNovedades);
                this.fueraConvenioDataSource.data = data.fueraConvenio.map(initNovedades);
                this.mensualDataSource.data = data.mensual.map(initNovedades);

                setTimeout(() => {
                    this.jornalesDataSource.paginator = this.jornalesPaginator;
                    this.fueraConvenioDataSource.paginator = this.fueraConvenioPaginator;
                    this.mensualDataSource.paginator = this.mensualPaginator;
                });

                this.toggleSaveButton(true);
                this.loading = false;
                this.panelOpenState = false;
                this._cdr.markForCheck();
            },
            error: (error) => {
                console.error(error);
                this._notificationService.notifyError(this.translate('novedades_contador_error_vista_previa'));
                this.loading = false;
                this._cdr.markForCheck();
            }
        });
    }

    guardarNovedades(): void {
        if (!this.novedadesData) return;
        if (this.saving) return;

        this.saving = true;
        this._cdr.markForCheck();
        this.toggleSaveButton(false, true);

        this._novedadesService.saveNovedades(this.novedadesData).subscribe({
            next: () => {
                this._notificationService.notifySuccess(this.translate('novedades_contador_exito_guardado'));
                this.saving = false;
                this.resetData();
                this.toggleSaveButton(false);
                this.panelOpenState = true;
                this._cdr.markForCheck();
            },
            error: (error) => {
                console.error(error);
                this._notificationService.notifyError(this.translate('novedades_contador_error_guardado'));
                this.saving = false;
                this.toggleSaveButton(true);
                this._cdr.markForCheck();
            }
        });
    }

    toggleExpand(element: any, type: string): void {
        switch (type) {
            case 'jornal': this.expandedJornal = this.expandedJornal === element ? null : element; break;
            case 'fuera': this.expandedFuera = this.expandedFuera === element ? null : element; break;
            case 'mensual': this.expandedMensual = this.expandedMensual === element ? null : element; break;
        }
    }

    isExpanded(element: any, type: string): boolean {
        switch (type) {
            case 'jornal': return this.expandedJornal === element;
            case 'fuera': return this.expandedFuera === element;
            case 'mensual': return this.expandedMensual === element;
            default: return false;
        }
    }

    private toggleSaveButton(show: boolean, disabled: boolean = false): void {
        this.actionStateChange.emit({
            key: 'novedades_contador_guardar',
            changes: {
                hidden: !show,
                disabled: disabled
            }
        });
    }
}